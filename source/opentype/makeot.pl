...
# having glyphs listed here which don't exist in a particular font
# causes .notdef to be added to the coverage table for the superscript diacritic feature
# having a superscript diac over .notdef wouldn't actually hurt anything
my @superscript = qw(
...
if (lc (substr (scalar $f->{'name'}->find_name(1), 0, 7)) eq 'andika') # name id 1 = Font Family name
	{ $andika_f = 1; }
...
foreach $n (@superscript)
	{$superscript_context_cover->add($gnames{$n})};
...
#foreach $n (keys %gnames) # order of keys is random
{
    ...
    elsif (exists $special_dotted{$glyph->{'post'}})
    {
...
{
	next if (not exists $gnames{$k}); # exclude glyphs not in the font being processed
...
my @gsub_lkups = qw(di_ctx di_sub di_csub c_sub vd_sub vc_sub srb_sub ma_sub sa_sub ss_ctx ss_sub 
	...
	rams_sub hstrk_sub itl_sub jsrf_sub jstrk_sub ltl_sub engs_sub nhk_sub opno_sub ou_sub 
...
'DFLT' => { #DFLT script needed where no script is specified, eg InDesign with no language selected
	'DEFAULT' => {
    	...
    	'FEATURES' => ['ccmp', 'aalt', 'smcp', 'c2sc', 'liga', 
			...
			'cv20', 'cv25', 'cv28', 'cv31', 'cv34', 'cv37', 'cv39', 
...
sub add_cv_feat
{
	...
	{
		# test that all glyphs exist since there may be some that are only in some fonts
		my $skip_glyphs = 0;
		if (not exists $gnames{$g->{'base'}}) {$skip_glyphs = 1;}
		foreach my $alt (@{$g->{'alts'}})
			{ if (not exists $gnames{$alt}) {$skip_glyphs = 1;} }
...
#  and correcting references in features to lookups that come after the deleted one
sub gsub_lookup_del
{
	my ($lkup_lbl) = @_;
